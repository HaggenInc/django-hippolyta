...
class S3BotoStorage(Storage):
    ...
    def __init__(self, bucket_name=BUCKET_NAME, 
        ...
        try:
            self.bucket = self.connection.get_bucket(bucket_name, validate=True, headers=headers)
    ...
    def url(self, name):
        ...
        return "http://%s.s3.amazonaws.com/%s" % (self.bucket.name, name)
...
class S3BotoStorageFile(File):
    ...
    def close(self):
        if self._is_dirty:
            self.key.set_contents_from_string(self.file.getvalue(), headers=self._storage.headers, policy=self._storage.acl)
        self.key.close()
